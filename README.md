...
This is **Reef**. It's a web-based platform that orchestrates your data workflows. It lets you run queries, transform results, and deliver them to multiple destinations, all from a single easy-to-use interface. Manage connections, profiles, and scheduled executions centrally. All low code, no scripts unless you need them.
...
To keep changes and events in the application traceable, we've included:

- Extensive execution history details, showing various information such as status, duration, row counts
- Application logging available both in the UI and the `/logs` folder
- Audit logging available in the admin-panel
- Health API endpoint available with a GET on `/health` for Azure Monitor, Prometheus, Grafana

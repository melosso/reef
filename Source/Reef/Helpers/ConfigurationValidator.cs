...
public static class ConfigurationValidator
{
    ...
    public static void ValidateConfiguration(IConfiguration configuration)
    {
        ...
        // Validate SMTP configuration (optional - only if configured)
        var smtpHost = configuration.GetValue<string>("Reef:Mail:SmtpHost");
        if (!string.IsNullOrWhiteSpace(smtpHost))
        {
            var smtpPort = configuration.GetValue<int?>("Reef:Mail:SmtpPort");
            if (!smtpPort.HasValue || smtpPort.Value <= 0 || smtpPort.Value > 65535)
            {
                Log.Warning("Reef:Mail:SmtpPort is invalid - email notifications will fail");
            }

            var smtpFrom = configuration.GetValue<string>("Reef:Mail:FromAddress");
            if (string.IsNullOrWhiteSpace(smtpFrom))
            {
                Log.Warning("Reef:Mail:FromAddress is not configured - email notifications will fail");
            }
        }

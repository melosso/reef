...
public class JobService
{
    ...
    public async Task<Job> CreateAsync(Job job)
    {
        ...
        var sql = @"
            INSERT INTO Jobs (
                ...
                ScheduleType, CronExpression, IntervalMinutes, StartDate, EndDate, StartTime, EndTime,
    ...
    ///
    /// IMPORTANT: Auto-pause logic explained
    ...
    private DateTime CalculateWeeklyNextRun(DateTime baseTime, TimeSpan? startTime, Job job)
    {
        ...
        var daysUntilTarget = ((int)targetDay - (int)baseTime.DayOfWeek + 7) % 7;
        
        // If today is the target day but the time has passed, wait until next week
        var now = DateTime.UtcNow;
        ...
        return baseTime.Date.AddDays(daysUntilTarget).Add(time);
    }

...
public class AdminService
{
    ...
    public AdminService(DatabaseConfig config, PasswordHasher passwordHasher, EncryptionService encryptionService)
    {
    ...
    public async Task<object> GetSystemMetricsAsync()
    {
        try
        {
            ...
            // Get uptime (from database creation time)
            var uptimeQuery = await conn.QueryFirstOrDefaultAsync<DateTime?>("SELECT CreatedAt FROM Users ORDER BY CreatedAt LIMIT 1");
